...
mlp_clf = MLPClassifier(random_state=1, solver='adam', hidden_layer_sizes=(128, 64, 2), \
...
fig.figure_.suptitle(r"$c_{H\widetilde{W}}/\Lambda^{2}=1/TeV^{2}$"+"| Accuracy={:.2f}%".format(accuracy_score(testY, y_pred)*100))
plt.show()
...
#axis.bar(bin_centres_ONN, hist_values3*100/bin_width_ONN, width=bin_width_ONN, alpha=0.2, color="red", label=r"$c_{H\widetilde{W}}/\Lambda^{2}=1/TeV^{2}$")
axis.errorbar(bin_centres_ONN, hist_values3*100/bin_width_ONN, yerr=n_err3*100/bin_width_ONN, fmt='o', color="purple")
...
axis.step(new_bin_centres_ONN+bin_width_ONN/2, new_hist_values*100/bin_width_ONN, color="purple", label=r"$c_{H\widetilde{W}}/\Lambda^{2}=1/TeV^{2}$")
# SM
...
c_deltaphi = np.linspace(-0.3,0.3,len(n_deltaphi))
c_ONN = np.linspace(-0.3,0.3,len(n_ONN))
...
axis.set_xlabel(r"$c_{H\widetilde{W}}/\Lambda^{2}[TeV^{-2}]$", fontsize = 35)
