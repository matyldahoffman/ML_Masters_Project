...
mlp_clf = MLPClassifier(random_state=1, solver='adam', hidden_layer_sizes=(128, 32, 2), \
                        max_iter = 100, activation = 'tanh', validation_fraction=0.2, early_stopping=True)
...
fig.figure_.suptitle(r"$c_{H\widetilde{B}}/\Lambda^{2}=1/TeV^{2}$"+" | Accuracy={:.2f}%".format(accuracy_score(testY, y_pred)*100))
plt.show()
...
axis.bar(bin_centres, 10*hist_values/bin_width, width=bin_width, alpha=0.2, color="red", label=r"$c_{H\widetilde{B}}/\Lambda^{2}=1/TeV^{2}$")
...
print('Original weighting factor SMEFT = ', weights[0]*100000)
print('Original weighting factor SM = ', weights_sm[0]/CROSS_SECTION_PER_EVENT_sm)
print('A SMEFT = ', '0.00001')
print('A SM = ', CROSS_SECTION_PER_EVENT_sm)
...
#axis.bar(bin_centres_ONN, hist_values3*100/bin_width_ONN, width=bin_width_ONN, alpha=0.2, color="red", label=r"$c_{H\widetilde{B}}/\Lambda^{2}=1/TeV^{2}$")
axis.errorbar(bin_centres_ONN, hist_values3*100/bin_width_ONN, yerr=n_err3*100/bin_width_ONN, fmt='o', color="crimson")
...
axis.step(new_bin_centres_ONN+bin_width_ONN/2, new_hist_values*100/bin_width_ONN, color="crimson", label=r"$c_{H\widetilde{B}}/\Lambda^{2}=1/TeV^{2}$")
# SM
...
print("length of lambda_deltaphi = ", len(EFT_deltaphi))
print("length of n_ONN = ", len(n_ONN))
print("length of lambda_ONN = ", len(EFT_ONN))

c_deltaphi = np.linspace(-0.5,0.5,len(n_deltaphi))
c_ONN = np.linspace(-0.5,0.5,len(n_ONN))
...
print("Chi square delta phi = \n", chi_square_deltaphi)
print("Chi square O_NN = \n", chi_square_ONN)
...
axis.set_xlabel(r"$c_{H\widetilde{B}}/\Lambda^{2}[TeV^{-2}]$", fontsize = 35)
...
print(coeffs_deltaphi[1]**2)
print(4*coeffs_deltaphi[0]*(coeffs_deltaphi[2]-1.96**2))
print(coeffs_deltaphi[1]**2-4*coeffs_deltaphi[0]*(coeffs_deltaphi[2]-1.96**2))
